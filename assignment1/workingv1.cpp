...
std::vector<int> getSub(std::vector<int> a, std::vector<int> b, int B){
    ...
    if (diff > 0){ // a > b 
        ...
        // swap vectors around so loop works 
        std::vector<int> temp = a;
        a = b;
        b = temp;
        a_is_bigger = 0;
    }
    else if (diff == 0){ // vectors are same length but check that a is bigger than b 
        ...
        if (a_is_bigger == 0){ // if b is bigger than a 
            // swap vectors around so loop works 
            std::vector<int> temp = a;
            a = b;
            b = temp;
    ...
    if (a_is_bigger == 0){ // if we swapped vectors at start we need to put a minus sign in front 
        sum.at(0) = -sum.at(0); 
    }
    return sum;
}
...
std::vector<int> getSum(std::vector<int> a, std::vector<int> b, int B){
    // ensure no number are negative, if they are, call getSub 
    ...
    if (sum.at(0) == 0){
        sum.erase(sum.begin());  // Erase the leading zero
    }
    return sum;
}
...
std::vector<int> getProduct(std::vector<int> a, std::vector<int> b, int B){
    ...
    // select smaller array for efficiency
...
int getComp(std::vector<int> a, std::vector<int> b, int B){
    // checks that a > b, returns 1 if a > b, returns 0 if b > a
    int diff = a.size() - b.size(); // difference in array sizes 
    if (diff > 0){ // a > b 
        return 1;
    }
...
std::vector<int> getDiv(std::vector<int> a, std::vector<int> b, int B){
    ...
    // while a > sum 
    while (getComp(a,sum,B)){
        sum = getSum(sum,b,B);
        count = getSum(count,one,B);
    }
...
int main(void){
    ...
    std::vector<int> div = getDiv(first,second,B);
